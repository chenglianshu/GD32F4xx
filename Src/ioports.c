/*
  ioports.c - driver code for GD32F4xx ARM processors
    ...
    if(output->id < digital.out.n_ports) {
        ...
        if(config->open_drain != aux_out[output->id].mode.open_drain) {
            if((aux_out[output->id].mode.open_drain = config->open_drain))
                gpio_output_options_set(aux_out[output->id].port, GPIO_OTYPE_OD, GPIO_OSPEED_50MHZ, 1U << aux_out[output->id].pin);
            else
                gpio_output_options_set(aux_out[output->id].port, GPIO_OTYPE_PP, GPIO_OSPEED_50MHZ, 1U << aux_out[output->id].pin);
        }
        if(persistent)
...
static bool digital_in_cfg (xbar_t *input, gpio_in_config_t *config, bool persistent)
{
    if(input->id < digital.in.n_ports && config->pull_mode != PullMode_UpDown) {
        ...
        else if(config->pull_mode == PullMode_Down)
            pupd = GPIO_PUPD_PULLDOWN;
        gpio_mode_set(aux_in[input->id].port, GPIO_MODE_INPUT, pupd, 1U << input->pin);
        if(persistent)
